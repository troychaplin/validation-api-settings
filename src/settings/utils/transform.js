const { blockTitles = {}, postTypeLabels = {} } = window.validationApiSettings || {};
		...
		for ( const [ postType, metaKeys ] of Object.entries( checks.meta ) ) {
			for ( const [ metaKey, metaChecks ] of Object.entries( metaKeys ) ) {
				for ( const [ checkName, check ] of Object.entries( metaChecks ) ) {
					...
					const override =
						settings?.meta?.[ postType ]?.[ metaKey ]?.[ checkName ] ?? null;
	...
	for ( const row of rows ) {
		...
		switch ( row.scope ) {
			...
			case 'meta':
				...
				settings.meta[ row.post_type ][ row.meta_key ][ row.check_name ] = row.level;
				break;
